...
RECORDLEN = 1030 #1024 # see config file
ADC_TO_V = 8192. # hard-coded conversion; depends on digitizer res and range
...
if (len(sys.argv) < 2 or len(sys.argv) > 3):
  print("USAGE: python plot_wavetxt.py <wave0.txt> [plotting off(0)/on(1)]")
  sys.exit(1)
...
def main():
  # open data file
  print("\nNOTE: ADC to V conversion is hard-coded!!\nReading file...")
  with open(FILE) as f:
    data = f.readlines()
#    data = [line.split(' ')[-1] for line in f] # reads last column of values into list
  ...
  for line in data[:10]:
    ...
    if (recordLength == 0): # if no header info
      print("NO HEADER DETECTED. Setting recordLength to default.")
  ...
  while j < len(data): # while we are not at the end of file
    ...
    # holderArray.append((float(data[j]) - (2**14)*0.86)/ADC_TO_V) # V; 0.86 hard coded, look in config file
...
#      print(ini_ped)
    ...
    holderArray.append((float(data[j]) - ini_ped/len(data[frameStart:frameStart + recordLength/10]))/ADC_TO_V) 
