// Light attentuation at 10 meters
/*
  * This macro extrapolates the light attenuation at
  * 10 cm to 10 m and convolves the result for 
  *
*/
...
TGraph *spec_graph(const char* filename)
{
...
std::vector<TGraph*> make_spect_graphs() {
  ...
  for (size_t i = 0; i < dir.GetListOfFiles()->GetEntries(); ++i) {
    ...
    {
      TGraph* tg = make_spect_graph(filename.c_str());
